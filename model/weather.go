...
type RequestWeatherByCoords struct {
...
func GetWeatherByCoords(req RequestWeatherByCoords, apiContext *ApiContext) (*Weather, error) {
	log.Printf("Requested weather for coords: %f, %f", req.Latitude, req.Longitude)
	var weather = &OpenWeather{}
	units := "metric"
	if req.Fahrenheit {
		units = "imperial"
	}
	url := fmt.Sprintf("https://api.openweathermap.org/data/2.5/weather?lat=%f&lon=%f&units=%s&appid=%s", req.Latitude, req.Longitude, units, apiContext.Key)
	res, err := http.Get(url)
	if err != nil {
		log.Printf("HTTP GET error: %s\n%v", url, err)
		return nil, err
